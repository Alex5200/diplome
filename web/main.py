...
Endpoints:
    ...
    POST /api/connect              — подключиться к роботу (требует токен)
    POST /api/disconnect           — отключиться (требует токен)
    POST /api/move                 — двигать сустав (требует токен)
    POST /api/stop                 — аварийная остановка (требует токен)
    POST /api/ik                   — inverse kinematics (требует токен)
