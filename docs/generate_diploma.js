...
children.push(
  ...
  p("  2.1 Общая архитектура системы", { noIndent: true, afterSpacing: 40 }),
  p("  2.2 Модель данных", { noIndent: true, afterSpacing: 40 }),
  p("  2.3 Контроллеры", { noIndent: true, afterSpacing: 40 }),
  p("  2.4 Графический интерфейс", { noIndent: true, afterSpacing: 40 }),
  p("  2.5 Система событий", { noIndent: true, afterSpacing: 60 }),
...
children.push(
  ...
  p("\u2014 создать графический интерфейс оператора в стиле промышленных контроллеров FANUC;", { noIndent: true }),
  p("\u2014 реализовать 3D-визуализацию робота и систему блочного программирования;", { noIndent: true }),
  ...
  p([{ text: "Практическая значимость ", bold: true }, { text: "работы состоит в создании готового к использованию программного комплекса, обеспечивающего управление реальным роботом через графический интерфейс, расчёт траекторий по координатам целевой точки и возможность интеграции с системами машинного обучения через среду MuJoCo." }]),
...
children.push(
  ...
  heading2("2.1 Общая архитектура системы"),

  p("Программный комплекс построен по модульной архитектуре MVC (Model\u2013View\u2013Controller) на языке Python 3.12+. Выбор архитектуры обусловлен необходимостью разделения логики управления роботом, представления данных и пользовательского интерфейса. Структура проекта представлена в таблице 2.1."),
  emptyLine(),
  tableCaption("Таблица 2.1 \u2014 Модульная структура проекта"),
  makeTable(
    ["Модуль", "Назначение"],
    [
      ["app/models/kinematics.py", "Прямая и обратная кинематика (FK/IK)"],
      ["app/models/motor_data.py", "Модели данных: MotorData, JointState, RobotState"],
      ["app/controllers/motor_controller.py", "Управление сервоприводами ST3215"],
      ["app/controllers/motor_monitor.py", "Асинхронный мониторинг моторов"],
      ["app/views/main_window.py", "Главное окно GUI (Tkinter)"],
      ["app/views/manual_control_panel.py", "Панель ручного управления"],
      ["app/views/kinematics_3d_panel.py", "3D-визуализация кинематики (Matplotlib)"],
      ["app/views/block_programming.py", "Блочное визуальное программирование"],
      ["app/views/motor_mapping_panel.py", "Настройка маппинга суставов и моторов"],
      ["app/core/events.py", "Шина событий EventBus (pub-sub)"],
      ["app/utils/program_executor.py", "Исполнитель блочных программ"],
      ["app/utils/config_manager.py", "Менеджер конфигурации (JSON)"],
      ["app/tests/safe_motor_visualizer.py", "Безопасный визуализатор с IK/waypoints"],
      ["app/tests/mujoco_robot_sim.py", "Симуляция в MuJoCo (MJCF, RL-среда)"],
    ],
    [3600, 5400]
  ),
  emptyLine(),

  p("Поток данных в системе организован следующим образом: пользователь взаимодействует с GUI (Views), который через контроллеры (Controllers) отправляет команды на уровень аппаратного взаимодействия с библиотекой st3215. Обратная связь поступает через MotorMonitor, который асинхронно опрашивает моторы каждые 0.5 секунды и обновляет модели данных. Система событий EventBus обеспечивает слабое связывание компонентов."),

  heading2("2.2 Модель данных"),

  p("Модель данных включает следующие ключевые классы:"),

  p([{ text: "MotorData ", bold: true }, { text: "\u2014 dataclass, хранящий текущее состояние одного сервопривода: position (0\u20134095), temperature (\u00B0C), voltage (В), current (мА), load (%), mode, moving, torque_enabled, error_count. Свойство status возвращает вычисленное состояние: OK, WARNING (температура \u2265 70\u00B0C или нагрузка \u2265 80%), ERROR (температура \u2265 80\u00B0C), DISCONNECTED." }]),

  p([{ text: "JointState ", bold: true }, { text: "\u2014 состояние одного сустава: index, name, angle_deg, position, motor_id, min_angle, max_angle. Содержит методы конвертации to_motor_position() и to_angle()." }]),

  p([{ text: "RobotState ", bold: true }, { text: "\u2014 агрегированное состояние робота: список JointState для 6 суставов, словарь MotorData для всех моторов, флаги is_connected и is_moving. Свойства all_motors_ok, any_motor_warning, any_motor_error для быстрой диагностики." }]),

  p([{ text: "ProgramBlock / RobotProgram ", bold: true }, { text: "\u2014 модели для блочного программирования. ProgramBlock содержит тип блока (move_to, wait_time, home, torque_on, torque_off), параметры и порядок выполнения. RobotProgram \u2014 именованная последовательность блоков с сериализацией в JSON." }]),

  heading2("2.3 Контроллеры"),

  p([{ text: "MotorController ", bold: true }, { text: "\u2014 основной класс управления моторами. Инкапсулирует экземпляр ST3215, обеспечивает потокобезопасный доступ через threading.Lock. Основные методы: connect()/disconnect() \u2014 подключение к COM-порту; scan_servos() \u2014 автообнаружение моторов; move_to_position(motor_id, position, speed, acc) \u2014 отправка команды MoveTo; read_motor_data(motor_id) \u2014 чтение телеметрии; emergency_stop_all() \u2014 экстренная остановка через StopServo для всех моторов." }]),

  p([{ text: "MotorMonitor ", bold: true }, { text: "\u2014 асинхронный мониторинг в фоновом потоке. Каждые 0.5 секунды опрашивает все подключённые моторы (ReadPosition, ReadTemperature, ReadVoltage, ReadCurrent, ReadLoad, IsMoving) и обновляет модель MotorData. При превышении 5 последовательных ошибок чтения генерирует предупреждение." }]),

  heading2("2.4 Графический интерфейс"),

  p("GUI реализован на Tkinter с визуальной стилизацией под промышленные контроллеры FANUC: тёмная цветовая схема (#1a1a2e \u2014 фон, #16213e \u2014 панели, #00ff88 \u2014 статус OK, #ff9500 \u2014 предупреждения, #ff4444 \u2014 ошибки). Интерфейс организован в виде вкладок:"),

  p([{ text: "1. Главная вкладка ", bold: true }, { text: "\u2014 подключение к COM-порту, сканирование моторов, статус соединения." }]),
  p([{ text: "2. Ручное управление ", bold: true }, { text: "\u2014 выбор сустава (J1\u2013J6), кнопки прямого/обратного хода, слайдер скорости (0\u2013100%), быстрые команды (Home, Center, Torque ON/OFF)." }]),
  p([{ text: "3. Маппинг моторов ", bold: true }, { text: "\u2014 настройка соответствия логических суставов физическим моторам с поддержкой инверсии." }]),
  p([{ text: "4. 3D-кинематика ", bold: true }, { text: "\u2014 6 полей ввода углов, 3D-визуализация робота через Matplotlib, кнопка применения углов к реальным моторам." }]),
  p([{ text: "5. Блочное программирование ", bold: true }, { text: "\u2014 палитра блоков (движение, ожидание, управление моментом), холст для составления программы, кнопки запуска/паузы/остановки." }]),

  p("Нижняя панель мониторинга отображает в реальном времени данные всех 6 моторов: позиция, температура, напряжение, ток, нагрузка, статус с цветовой индикацией."),

  heading2("2.5 Система событий"),

  p("Для слабого связывания компонентов реализована шина событий EventBus по паттерну publish\u2013subscribe. Класс EventBus \u2014 синглтон, предоставляющий методы on(event_name, callback, priority) для подписки, emit(event_name, data) для публикации и off(event_name, callback) для отписки. BaseService \u2014 абстрактный базовый класс сервисов с жизненным циклом initialize \u2192 start \u2192 stop."),
