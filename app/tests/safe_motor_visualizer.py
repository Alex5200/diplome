...
class SafeMotorVisualizer:
    ...
    def __init__(self):
        ...
        self.st3215: ST3215 | None = None  # Прямой экземпляр ST3215
        self.device = "COM3"
    ...
    def _create_sliders(self):
        ...
        for i in range(6):
            ...
            move_btn = ttk.Button(
                frame, text="➡️", width=3, command=lambda idx=i: self._move_joint(idx)
            )
    ...
    def _create_target_panel(self):
        ...
        # X
        ttk.Label(input_row, text="X:", font=("Consolas", 10, "bold"), foreground="#FF6B6B").pack(
            side="left", padx=2
        )
        ...
        # Y
        ttk.Label(input_row, text="Y:", font=("Consolas", 10, "bold"), foreground="#4ECDC4").pack(
            side="left", padx=(10, 2)
        )
        ...
        # Z
        ttk.Label(input_row, text="Z:", font=("Consolas", 10, "bold"), foreground="#45B7D1").pack(
            side="left", padx=(10, 2)
        )
        ...
        ttk.Button(btn_row, text="🧮 Рассчитать IK", command=self._solve_ik_for_target).pack(
            side="left", padx=3
        )
        ttk.Button(btn_row, text="🎯 Двигать к точке", command=self._move_to_target_point).pack(
            side="left", padx=3
        )
        ttk.Button(btn_row, text="📌 Показать на 3D", command=self._show_target_on_3d).pack(
            side="left", padx=3
        )
        ...
        preset_frame = ttk.LabelFrame(
            target_outer, text="✅ Проверенные точки (IK работает)", padding=5
        )
        ...
        preset_scroll = ttk.Scrollbar(
            preset_list_frame, orient="vertical", command=self.preset_listbox.yview
        )
        ...
        ttk.Button(
            preset_btn_row, text="➕ Сохранить текущую", command=self._save_preset_point
        ).pack(side="left", padx=2)
        ttk.Button(preset_btn_row, text="➖ Удалить", command=self._remove_preset_point).pack(
            side="left", padx=2
        )
        ttk.Button(preset_btn_row, text="🗑️ Очистить", command=self._clear_preset_points).pack(
            side="left", padx=2
        )
        ttk.Button(preset_btn_row, text="🎯 Применить", command=self._apply_preset_point).pack(
            side="left", padx=5
        )
        ...
        wp_scroll = ttk.Scrollbar(wp_list_frame, orient="vertical", command=self.wp_listbox.yview)
        ...
        ttk.Button(wp_btn_row, text="➕ Добавить", command=self._add_waypoint).pack(
            side="left", padx=2
        )
        ttk.Button(wp_btn_row, text="➖ Удалить", command=self._remove_waypoint).pack(
            side="left", padx=2
        )
        ttk.Button(wp_btn_row, text="🗑️ Очистить", command=self._clear_waypoints).pack(
            side="left", padx=2
        )
        ttk.Button(wp_btn_row, text="▶️ Обход всех", command=self._run_waypoints).pack(
            side="left", padx=5
        )
        ttk.Button(wp_btn_row, text="🎯 К выбранной", command=self._go_to_selected_waypoint).pack(
            side="left", padx=2
        )
    ...
    def _create_buttons(self):
        ...
        # Кнопки подключения
        ttk.Button(btn_frame, text="🔌 Подключить", command=self._connect).pack(side="left", padx=5)

        ttk.Button(btn_frame, text="🔌 Отключить", command=self._disconnect).pack(
            side="left", padx=5
        )

        ttk.Button(btn_frame, text="🔍 Сканировать моторы", command=self._scan_motors).pack(
            side="left", padx=5
        )
        ...
        self.emergency_btn = ttk.Button(
            btn_frame, text="🛑 ЭКСТРЕННАЯ ОСТАНОВКА", command=self._emergency_stop
        )
    ...
    def _solve_ik_for_target(self):
        ...
        if angles is None:
            self.ik_status_var.set(
                f"❌ IK не сошлась для ({x:.0f}, {y:.0f}, {z:.0f}). Попробуйте другую точку."
            )
            self._draw_robot()
            return None
        ...
        self.ik_status_var.set(
            f"✅ IK решена! Точка ({x:.0f}, {y:.0f}, {z:.0f}) → ошибка: {error:.1f} мм{clamp_warn}"
        )
    ...
    def _run_waypoints_thread(self):
        ...
        for i, (x, y, z) in enumerate(self.waypoints):
            ...
            self.root.after(
                0,
                lambda idx=i, xx=x, yy=y, zz=z: self._waypoint_step(idx, xx, yy, zz, total),
            )
            ...
            if distance > max_reach:
                self.root.after(
                    0,
                    lambda idx=i: self.info_text.insert(
            ...
            if self.is_connected and self.st3215:
                for j in range(6):
                    ...
                    try:
                        self.st3215.MoveTo(
                            motor_id, position, speed=self.SAFE_SPEED, acc=DEFAULT_ACC
                        )
                    except Exception as e:
                        self.root.after(
                            0,
                            lambda idx=i, err=e: self.info_text.insert(
            ...
            self.root.after(
                0,
                lambda idx=i: self.info_text.insert(tk.END, f"✅ Точка #{idx + 1} достигнута\n"),
            )

        self.is_moving = False
        self.root.after(
            0,
            lambda: self.ik_status_var.set(f"✅ Обход маршрута завершён ({total} точек)"),
        )
    ...
    def _connect(self):
        ...
        try:
            ...
            self.conn_label.configure(text="✅ Подключено", foreground="#00ff88")
            messagebox.showinfo(
                "Успех",
                f"Подключено к {port}\n\n"
    ...
    def _disconnect(self):
        ...
        self.st3215 = None
        self.is_connected = False
        self.conn_label.configure(text="❌ Отключено", foreground="red")
    ...
    def _move_joint(self, idx):
        ...
        self.is_moving = True
        try:
            self.st3215.MoveTo(motor_id, position, speed=self.SAFE_SPEED, acc=DEFAULT_ACC)
    ...
    def _apply_all_angles(self):
        ...
        # Подтверждение
        angles_text = "\n".join(
            [f"  {self.JOINT_NAMES[i]}: {self.target_angles[i]:.1f}°" for i in range(6)]
        )
    ...
    def _emergency_stop(self):
        ...
        if messagebox.askyesno(
            "🛑 ЭКСТРЕННАЯ ОСТАНОВКА", "Остановить ВСЕ моторы?\n\nЭто отключит момент!"
        ):
            try:
                ...
                if not self.found_servos:
                    for i in range(6):
                        motor_id = self._get_motor_id_for_joint(i)
                        self.st3215.StopServo(motor_id)
                self.conn_label.configure(text="⚠️ ОСТАНОВЛЕНО", foreground="orange")
    ...
    def _update_info(self):
        ...
        if self.target_point:
            ...
            target_dist = math.sqrt(
                (end_pos[0] - tx) ** 2 + (end_pos[1] - ty) ** 2 + (end_pos[2] - tz) ** 2
            )
            self.info_text.insert(
                tk.END,
                f"\n=== ЦЕЛЕВАЯ ТОЧКА ===\n\n"
    ...
    def _on_closing(self):
        ...
        if self.is_connected:
            if messagebox.askyesno("Закрытие", "Отключить моторы перед закрытием?"):
    ...
    def run(self):
        ...
        print("  - Углы ограничены безопасными диапазонами")
        ...
        print("\n🎯 Новые функции:")
        print("  - Ввод целевой точки XYZ → обратная кинематика")
        print("  - Клик по 3D визуализации (включите галочку)")
        print("  - Маршрутные точки (waypoints) с последовательным обходом")
        print("\n" + "=" * 70)
        self.root.mainloop()
