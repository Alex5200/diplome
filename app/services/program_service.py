...
class ProgramService(BaseService):
    ...
    def __init__(self, robot_service=None):
        ...
        self._current_program: Optional[RobotProgram] = None
        self._execution_thread: Optional[threading.Thread] = None
        self._is_running = False
        self._is_paused = False
        self._current_block_index = 0
        self._execution_callback: Optional[Callable[[str, Any], None]] = None
    ...
    def _execute_program(self) -> None:
        ...
        while self._is_running and self._current_block_index < len(blocks):
            ...
            self._emit_event('block_started', {
            ...
            self._emit_event('block_completed', {
                'index': self._current_block_index,
                'success': success,
            })
        ...
        if self._is_running:
            self._is_running = False
            self._emit_event('program_completed', {
    ...
    def _execute_block(self, block: ProgramBlock) -> bool:
        ...
        try:
            if block_type == 'move_to':
                ...
                self._emit_event('block_error', {
                    'error': f'Неизвестный тип блока: {block_type}'
                })
                return False
