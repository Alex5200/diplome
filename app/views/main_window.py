...
from app.config.constants import (
    MONITOR_INTERVAL, FANUC_BG, FANUC_PANEL, FANUC_GREEN,
    FANUC_ORANGE, FANUC_RED, FANUC_BLUE, FANUC_TEXT, FANUC_GRAY,
    MAX_POSITION, DEFAULT_SPEED,
    JOG_MODE_JOINT, JOG_MODE_CARTESIAN,
    SPEED_OVERRIDE_DEFAULT, SPEED_OVERRIDE_MIN, SPEED_OVERRIDE_MAX,
...
class FANUCStatusBar(tk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        super().__init__(parent, bg='#0a0a1a', height=36, **kwargs)
        self.pack_propagate(False)

        # Режим работы
        self.mode_label = tk.Label(
            self, text="JOINT", font=('Consolas', 11, 'bold'),
            bg='#0a0a1a', fg=FANUC_GREEN, padx=8
        )
        ...
        self.speed_pct_label = tk.Label(
            ...
            bg='#0a0a1a', fg=FANUC_ORANGE, padx=8
        )
        ...
        self.coord_label = tk.Label(
            ...
            bg='#0a0a1a', fg=FANUC_BLUE, padx=8
        )
        ...
        self.prog_status_label = tk.Label(
            ...
            bg='#0a0a1a', fg=FANUC_GRAY, padx=8
        )
        ...
        self.conn_canvas = tk.Canvas(
            self, width=14, height=14, bg='#0a0a1a', highlightthickness=0
        )
    ...
    def set_prog_status(self, status: str):
        colors = {
            'IDLE': FANUC_GRAY, 'RUN': FANUC_GREEN,
            'PAUSE': FANUC_ORANGE, 'ABORT': FANUC_RED,
            'DONE': FANUC_BLUE
        }
        self.prog_status_label.config(
            text=status, fg=colors.get(status, FANUC_TEXT)
        )
...
class PositionRegisterPanel(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        tk.Label(
            header, text="POSITION REGISTERS (PR)",
            font=('Consolas', 14, 'bold'), bg=FANUC_PANEL, fg=FANUC_GREEN
        ...
        tk.Button(
            btn_frame, text="RECORD", font=('Arial', 9, 'bold'),
            bg=FANUC_GREEN, fg='black', bd=0, padx=12, pady=4,
            command=self._record_current
        ).pack(side='left', padx=3)

        tk.Button(
            btn_frame, text="MOVE TO", font=('Arial', 9, 'bold'),
            bg=FANUC_BLUE, fg='white', bd=0, padx=12, pady=4,
            command=self._move_to_selected
        ).pack(side='left', padx=3)

        tk.Button(
            btn_frame, text="DELETE", font=('Arial', 9, 'bold'),
            bg=FANUC_RED, fg='white', bd=0, padx=12, pady=4,
            command=self._delete_selected
        ).pack(side='left', padx=3)

        tk.Button(
            btn_frame, text="CLEAR ALL", font=('Arial', 9, 'bold'),
        ...
        for i, (label, color) in enumerate([
            ...
            tk.Label(
                coords_frame, text=f"{label}:", font=('Consolas', 11, 'bold'),
                bg=FANUC_PANEL, fg=color
            ).grid(row=0, column=i * 2, padx=(10, 2))

            var = tk.DoubleVar(value=0.0)
            self.xyz_vars[label.lower()] = var
            ttk.Spinbox(
                coords_frame, from_=-400, to=400, textvariable=var,
                width=8, font=('Consolas', 11), increment=5.0
            ).grid(row=0, column=i * 2 + 1, padx=(2, 10))

        tk.Button(
            coords_frame, text="CALC IK + RECORD", font=('Arial', 9, 'bold'),
            bg='#7B68EE', fg='white', bd=0, padx=12, pady=4,
        ...
        col_widths = {
            'pr': 50, 'j1': 65, 'j2': 65, 'j3': 65,
            'j4': 65, 'j5': 65, 'j6': 65,
            'x': 70, 'y': 70, 'z': 70, 'comment': 150
        }
        col_titles = {
            'pr': 'PR#', 'j1': 'J1', 'j2': 'J2', 'j3': 'J3',
            'j4': 'J4', 'j5': 'J5', 'j6': 'J6',
        ...
        self.status_label = tk.Label(
            main, text="0 registers", font=('Consolas', 10),
    ...
    def _refresh_table(self):
        ...
        for pr_idx in sorted(self.registers.keys()):
            ...
            self.tree.insert('', 'end', values=(
                pr_idx,
                f"{angles[0]:.1f}", f"{angles[1]:.1f}", f"{angles[2]:.1f}",
                f"{angles[3]:.1f}", f"{angles[4]:.1f}", f"{angles[5]:.1f}",
...
class TeachPendantPanel(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        for text, bg, fg, cmd in buttons:
            tk.Button(
                btn_row, text=text, font=('Arial', 10, 'bold'),
                bg=bg, fg=fg, bd=0, padx=14, pady=6,
                command=cmd
        ...
        tk.Label(
            settings_frame, text="Delay (s):", font=('Arial', 10),
            bg=FANUC_PANEL, fg=FANUC_TEXT
        ).pack(side='left', padx=5)

        self.delay_var = tk.DoubleVar(value=0.5)
        ttk.Spinbox(
            settings_frame, from_=0.1, to=10.0, textvariable=self.delay_var,
            width=6, increment=0.1, font=('Consolas', 10)
        ).pack(side='left', padx=5)

        tk.Label(
            settings_frame, text="Loops:", font=('Arial', 10),
            bg=FANUC_PANEL, fg=FANUC_TEXT
        ).pack(side='left', padx=(20, 5))

        self.loop_var = tk.IntVar(value=1)
        ttk.Spinbox(
            settings_frame, from_=1, to=100, textvariable=self.loop_var,
            width=5, font=('Consolas', 10)
        ).pack(side='left', padx=5)

        self.play_status = tk.Label(
            settings_frame, text="IDLE", font=('Consolas', 10, 'bold'),
        ...
        tk.Button(
            del_frame, text="DELETE SELECTED", font=('Arial', 9, 'bold'),
            bg=FANUC_RED, fg='white', bd=0, padx=10, pady=3,
            command=self._delete_selected
        ).pack(side='left')

        self.count_label = tk.Label(
            del_frame, text="0 points", font=('Consolas', 10),
...
class AlarmHistoryPanel(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        tk.Label(
            header, text="ALARM HISTORY",
            font=('Consolas', 14, 'bold'), bg=FANUC_PANEL, fg=FANUC_RED
        ).pack(side='left', padx=10, pady=5)

        tk.Button(
            header, text="CLEAR", font=('Arial', 9, 'bold'),
            bg=FANUC_ORANGE, fg='white', bd=0, padx=10, pady=3,
            command=self._clear
...
class RobotControlGUI(tk.Tk):
    ...
    def _setup_styles(self):
        ...
        style.configure('Header.TLabel', font=('Arial', 14, 'bold'), foreground=FANUC_GREEN, background=FANUC_BG)
        style.configure('TButton', font=('Arial', 10, 'bold'), padding=5)
        style.configure('Connect.TButton', font=('Arial', 12, 'bold'), foreground='white', background=FANUC_BLUE, padding=10)
        style.configure('Emergency.TButton', font=('Arial', 14, 'bold'), foreground='white', background='#cc0000', padding=15)
        style.configure('TLabelframe', background=FANUC_PANEL, foreground=FANUC_TEXT)
        style.configure('TLabelframe.Label', background=FANUC_PANEL, foreground=FANUC_GREEN, font=('Arial', 11, 'bold'))
    ...
    def _create_menu(self):
        ...
        view_menu.add_command(label="Jog Mode: Joint", command=lambda: self._set_jog_mode(JOG_MODE_JOINT))
        view_menu.add_command(label="Jog Mode: Cartesian", command=lambda: self._set_jog_mode(JOG_MODE_CARTESIAN))
    ...
    def _create_widgets(self):
        ...
        bottom_frame.pack(fill='x', padx=4, pady=(0, 4))
    ...
    def _create_sidebar(self, parent) -> tk.Frame:
        ...
        sidebar = tk.Frame(parent, bg='#0d0d20', width=60)
        ...
        for text, bg, cmd, tooltip in buttons:
            btn = tk.Button(
                sidebar, text=text, font=('Consolas', 8, 'bold'),
                bg=bg, fg='white', bd=0, width=5, height=2,
        ...
        # E-STOP
        estop = tk.Button(
            sidebar, text="STOP", font=('Consolas', 8, 'bold'),
            bg='#cc0000', fg='white', bd=0, width=5, height=2,
        ...
        tk.Label(
            sidebar, text="SPD", font=('Consolas', 7, 'bold'),
            bg='#0d0d20', fg=FANUC_ORANGE
        ).pack()

        self.speed_override_var = tk.IntVar(value=SPEED_OVERRIDE_DEFAULT)
        speed_scale = tk.Scale(
            sidebar, from_=SPEED_OVERRIDE_MAX, to=SPEED_OVERRIDE_MIN,
            variable=self.speed_override_var, orient='vertical',
            bg='#0d0d20', fg=FANUC_ORANGE, troughcolor=FANUC_PANEL,
            highlightthickness=0, bd=0, width=12, length=120,
            font=('Consolas', 7),
        ...
        self.speed_pct_sidebar = tk.Label(
            sidebar, text="50%", font=('Consolas', 8, 'bold'),
            bg='#0d0d20', fg=FANUC_ORANGE
        )
    ...
    def _add_tooltip(widget, text):
        ...
        def show(event):
            ...
            tk.Label(
                tip, text=text, bg='#ffffe0', fg='black',
                font=('Arial', 9), relief='solid', bd=1, padx=4, pady=2
            ).pack()
    ...
    def _create_dashboard_tab(self):
        ...
        self.status_label = tk.Label(
            ...
            font=('Consolas', 14, 'bold'), bg=FANUC_PANEL, fg=FANUC_ORANGE
        )
        ...
        for i in range(6):
            indicator = tk.Canvas(axis_frame, width=28, height=28, bg=FANUC_PANEL, highlightthickness=0)
        ...
        tk.Label(
            port_frame, text="COM Port:", font=('Consolas', 11, 'bold'),
        ...
        tk.Button(
            port_frame, text="REFRESH", font=('Arial', 9, 'bold'),
            bg=FANUC_GRAY, fg='white', bd=0, padx=10, pady=4,
            command=self._refresh_ports
        ).pack(side='left', padx=5)

        self.connect_btn = tk.Button(
            port_frame, text="CONNECT", font=('Arial', 11, 'bold'),
            bg=FANUC_BLUE, fg='white', bd=0, padx=20, pady=6,
            command=self._toggle_connection
        )
        self.connect_btn.pack(side='left', padx=10)

        self.scan_btn = tk.Button(
            port_frame, text="SCAN SERVOS", font=('Arial', 11, 'bold'),
            bg=FANUC_ORANGE, fg='white', bd=0, padx=20, pady=6,
            state='disabled', command=self._scan_servos
        )
        ...
        for text, cmd, color in actions:
            tk.Button(
                quick_inner, text=text, font=('Arial', 10, 'bold'),
                bg=color, fg='white', bd=0, padx=18, pady=8,
                command=cmd
            ).pack(side='left', padx=4)
        ...
        for label, var_name, color in [
            ...
            tk.Label(
                coord_inner, text=f"{label}:", font=('Consolas', 12, 'bold'),
                bg=FANUC_PANEL, fg=color
            ).pack(side='left', padx=(12, 2))

            lbl = tk.Label(
                coord_inner, text="0.00", font=('Consolas', 13, 'bold'),
                bg='#0a0a1a', fg=FANUC_TEXT, padx=8, pady=4, relief='sunken'
            )
        ...
        for i in range(6):
            tk.Label(
                joints_inner, text=f"J{i+1}:", font=('Consolas', 11, 'bold'),
                bg=FANUC_PANEL, fg=FANUC_GREEN
            ).pack(side='left', padx=(12, 2))

            lbl = tk.Label(
                joints_inner, text="0.0", font=('Consolas', 12),
                bg='#0a0a1a', fg=FANUC_TEXT, padx=6, pady=3, relief='sunken', width=8
            )
    ...
    def _create_block_tab(self):
        ...
        for text, cmd, color in [
            ...
            tk.Button(
                btn_frame, text=text, font=('Arial', 10, 'bold'),
                bg=color, fg='white', bd=0, padx=14, pady=5,
                command=cmd
            ).pack(side='left', padx=3)
    ...
    def _create_coordinates_tab(self):
        ...
        tk.Button(
            btn_frame, text="REFRESH", font=('Arial', 10, 'bold'),
            bg=FANUC_BLUE, fg='white', bd=0, padx=14, pady=5,
            command=self._update_tool_coordinates
        ).pack(side='left', padx=5)

        tk.Button(
            btn_frame, text="ZERO", font=('Arial', 10, 'bold'),
            bg=FANUC_ORANGE, fg='white', bd=0, padx=14, pady=5,
    ...
    def _create_coordinate_row(self, parent, label, variable, color):
        ...
        ttk.Label(
            row, text=label, font=('Consolas', 13, 'bold'),
            foreground=color, width=5
        ).pack(side='left', padx=10)

        ttk.Label(
            row, textvariable=variable, font=('Consolas', 16, 'bold'),
            foreground=FANUC_TEXT, background='#0a0a1a', padding=10
    ...
    def _create_log_panel(self):
        self.log_text = scrolledtext.ScrolledText(
            self.log_frame, height=3, font=('Consolas', 9),
            state='disabled', bg='#0a0a14', fg=FANUC_TEXT,
            insertbackground='white', selectbackground=FANUC_BLUE
        )
        ...
        tk.Button(
            btn_row, text="CLEAR LOG", font=('Arial', 8, 'bold'),
    ...
    def _draw_axis_indicator(self, canvas, axis, state):
        canvas.delete('all')
        colors = {'active': FANUC_GREEN, 'inactive': FANUC_GRAY, 'warning': FANUC_ORANGE}
    ...
    def _import_registers(self):
        if self.position_register_panel:
            path = filedialog.askopenfilename(
                filetypes=[("JSON", "*.json")],
                title="Import Position Registers"
            )
            if path:
                try:
