...
class RobotControlGUI(ctk.CTk):
    ...
    def _go_all_home(self):
        ...
        if messagebox.askyesno("Confirm", "Move all joints to home position (0°)?"):
            ...
            self._log("All joints → home (0°)", "info")
    ...
    def _zero_and_release(self):
        """Move all joints to 0 rad (position 2048), then disable torque."""
        if not self.controller.connected:
            messagebox.showwarning("Warning", "Connect first!")
            return
        if not messagebox.askyesno(
            "Set zero position",
            "Move all joints to 0° and disable torque?\n\nAfter this you can freely position the arm by hand.",
        ):
            return
