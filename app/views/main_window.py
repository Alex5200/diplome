...
class FANUCStatusBar(ctk.CTkFrame):
    ...
    def __init__(self, parent, **kwargs):
        super().__init__(parent, fg_color=FANUC_BG, height=36,
        ...
        self.mode_label = ctk.CTkLabel(
            ...
            text_color=_ACCENT, fg_color=FANUC_BG,
        )
        ...
        self.speed_pct_label = ctk.CTkLabel(
            ...
            text_color=FANUC_ORANGE, fg_color=FANUC_BG,
        )
        ...
        self.coord_label = ctk.CTkLabel(
            self, text="WORLD",
            font=_F("Consolas", 10),
            text_color=FANUC_BLUE, fg_color=FANUC_BG,
        )
        ...
        self.prog_status_label = ctk.CTkLabel(
            self, text="IDLE",
            font=_F("Consolas", 10),
            text_color=FANUC_GRAY, fg_color=FANUC_BG,
        )
        ...
        self.conn_canvas = tk.Canvas(
            self, width=12, height=12, bg=FANUC_BG, highlightthickness=0,
        )
    ...
    def set_prog_status(self, status: str):
        colors = {"IDLE": FANUC_GRAY, "RUN": _ACCENT, "PAUSE": FANUC_ORANGE,
                  "ABORT": FANUC_RED, "DONE": FANUC_BLUE}
        self.prog_status_label.configure(
            text=status, text_color=colors.get(status, FANUC_TEXT))

    def set_connected(self, connected: bool):
        self.conn_canvas.delete("all")
        self.conn_canvas.create_oval(
            1, 1, 11, 11,
            fill=_ACCENT if connected else FANUC_RED, outline="",
        )
...
class PositionRegisterPanel(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        # Header + buttons
        hdr = ctk.CTkFrame(main, fg_color=_CARD_BG, corner_radius=10,
                           border_width=1, border_color=_BORDER)
        hdr.pack(fill="x", pady=(0, 10))

        ctk.CTkLabel(hdr, text="POSITION REGISTERS (PR)",
                     font=ctk.CTkFont("Segoe UI", 14, "bold"),
                     text_color=_ACCENT).pack(side="left", padx=14, pady=10)
        ...
        for txt, cmd, fg, hov in [
            ...
            ctk.CTkButton(hdr, text=txt, fg_color=fg, hover_color=hov,
                          text_color=FANUC_TEXT, width=90, height=30,
                          corner_radius=8, font=ctk.CTkFont("Segoe UI", 9, "bold"),
        ...
        for i, (label, color) in enumerate([("X", FANUC_RED), ("Y", _ACCENT), ("Z", FANUC_BLUE)]):
            ctk.CTkLabel(xyz_row, text=f"{label}:",
                         font=ctk.CTkFont("Consolas", 12, "bold"),
                         text_color=color).grid(row=0, column=i * 2, padx=(10, 2))
            var = tk.DoubleVar(value=0.0)
            self.xyz_vars[label.lower()] = var
            ttk.Spinbox(xyz_row, from_=-400, to=400, textvariable=var,
                        width=8, increment=5.0, font=("Consolas", 11),
                        ).grid(row=0, column=i * 2 + 1, padx=(2, 10))

        ctk.CTkButton(xyz_row, text="CALC IK + RECORD",
                      fg_color="#b8a8f8", hover_color="#8878d8",
                      text_color=FANUC_TEXT, width=140, height=32,
                      corner_radius=8, font=ctk.CTkFont("Segoe UI", 9, "bold"),
                      command=self._record_from_xyz).grid(row=0, column=6, padx=10)

        # Treeview
        tree_frame = ctk.CTkFrame(main, fg_color=_CARD_BG, corner_radius=10,
                                  border_width=1, border_color=_BORDER)
        ...
        widths = {"pr": 50, "j1": 65, "j2": 65, "j3": 65, "j4": 65,
                  "j5": 65, "j6": 65, "x": 70, "y": 70, "z": 70, "comment": 150}
        titles = {"pr": "PR#", "j1": "J1", "j2": "J2", "j3": "J3",
                  "j4": "J4", "j5": "J5", "j6": "J6",
                  "x": "X mm", "y": "Y mm", "z": "Z mm", "comment": "Comment"}
        for col in columns:
        ...
        self.status_label = ctk.CTkLabel(main, text="0 registers",
                                         font=ctk.CTkFont("Segoe UI", 9),
                                         text_color=FANUC_GRAY, anchor="w")
    ...
    def _refresh_table(self):
        ...
        for pr_idx in sorted(self.registers):
            ...
            self.tree.insert("", "end", values=(
                pr_idx, f"{a[0]:.1f}", f"{a[1]:.1f}", f"{a[2]:.1f}",
                f"{a[3]:.1f}", f"{a[4]:.1f}", f"{a[5]:.1f}",
                f"{xyz[0]:.1f}", f"{xyz[1]:.1f}", f"{xyz[2]:.1f}",
                reg.get("comment", ""),
            ))
...
class TeachPendantPanel(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        hdr = ctk.CTkFrame(main, fg_color=_CARD_BG, corner_radius=10,
                           border_width=1, border_color=_BORDER)
        hdr.pack(fill="x", pady=(0, 10))
        ctk.CTkLabel(hdr, text="TEACH PENDANT",
                     font=ctk.CTkFont("Segoe UI", 14, "bold"),
                     text_color=_ACCENT).pack(side="left", padx=14, pady=10)
        ...
        for txt, cmd, fg, hov in [
            ...
            ctk.CTkButton(btn_row, text=txt, fg_color=fg, hover_color=hov,
                          text_color=FANUC_TEXT, height=34, corner_radius=8,
                          font=ctk.CTkFont("Segoe UI", 10, "bold"),
                          command=cmd).pack(side="left", padx=3, pady=8)

        # Settings row
        settings_row = ctk.CTkFrame(ctrl_card, fg_color=_CARD_BG, corner_radius=0)
        settings_row.pack(fill="x", padx=10, pady=(0, 10))

        ctk.CTkLabel(settings_row, text="Delay (s):",
                     font=ctk.CTkFont("Segoe UI", 10),
                     text_color=FANUC_TEXT2).pack(side="left", padx=5)
        self.delay_var = tk.DoubleVar(value=0.5)
        ttk.Spinbox(settings_row, from_=0.1, to=10.0, textvariable=self.delay_var,
                    width=6, increment=0.1, font=("Consolas", 10)).pack(side="left", padx=5)

        ctk.CTkLabel(settings_row, text="Loops:",
                     font=ctk.CTkFont("Segoe UI", 10),
                     text_color=FANUC_TEXT2).pack(side="left", padx=(20, 5))
        self.loop_var = tk.IntVar(value=1)
        ttk.Spinbox(settings_row, from_=1, to=100, textvariable=self.loop_var,
                    width=5, font=("Consolas", 10)).pack(side="left", padx=5)

        self.play_status = ctk.CTkLabel(
            settings_row, text="IDLE",
        ...
        # Treeview
        tree_card = ctk.CTkFrame(main, fg_color=_CARD_BG, corner_radius=10,
                                 border_width=1, border_color=_BORDER)
        ...
        ctk.CTkButton(bot, text="DELETE SELECTED",
                      fg_color=FANUC_RED, hover_color="#c07065",
                      text_color=FANUC_TEXT, height=30, corner_radius=8,
                      font=ctk.CTkFont("Segoe UI", 9, "bold"),
                      command=self._delete_selected).pack(side="left")
        self.count_label = ctk.CTkLabel(bot, text="0 points",
                                        font=ctk.CTkFont("Segoe UI", 9),
                                        text_color=FANUC_GRAY)
    ...
    def _refresh_table(self):
        ...
        for i, pt in enumerate(self.teach_points):
            ...
            self.tree.insert("", "end", values=(
                i + 1, f"{a[0]:.1f}", f"{a[1]:.1f}", f"{a[2]:.1f}",
                f"{a[3]:.1f}", f"{a[4]:.1f}", f"{a[5]:.1f}",
                f"{xyz[0]:.1f}", f"{xyz[1]:.1f}", f"{xyz[2]:.1f}",
            ))
...
class AlarmHistoryPanel(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        ...
        hdr.pack(fill="x")
        ctk.CTkLabel(hdr, text="ALARM HISTORY",
                     font=ctk.CTkFont("Segoe UI", 14, "bold"),
                     text_color=FANUC_RED).pack(side="left", padx=12, pady=8)
        ctk.CTkButton(hdr, text="CLEAR",
                      fg_color=FANUC_ORANGE, hover_color="#d4995a",
                      text_color=FANUC_TEXT, width=70, height=28,
                      corner_radius=8, font=ctk.CTkFont("Segoe UI", 9, "bold"),
                      command=self._clear).pack(side="right", padx=10, pady=8)
...
class RobotControlGUI(ctk.CTk):
    ...
    def __init__(self):
        ...
        scale_pct    = max(MIN_SCALE_PERCENT, min(MAX_SCALE_PERCENT,
                           (sw / BASE_WINDOW_WIDTH) * 100))
        ...
        self.minsize(int(BASE_WINDOW_WIDTH  * MIN_SCALE_PERCENT / 100),
    ...
    def _setup_ttk_styles(self):
        s = ttk.Style()
        s.theme_use("clam")
        s.configure("Treeview", background=_CARD_BG, foreground=FANUC_TEXT,
                    rowheight=26, fieldbackground=_CARD_BG, borderwidth=0,
                    font=("Segoe UI", 9))
        s.configure("Treeview.Heading", background=FANUC_BG, foreground=FANUC_TEXT,
                    font=("Segoe UI", 9, "bold"), relief="flat")
        s.map("Treeview", background=[("selected", _ACCENT)],
              foreground=[("selected", FANUC_TEXT)])
        s.configure("TSpinbox", fieldbackground=_CARD_BG, foreground=FANUC_TEXT,
                    bordercolor=_BORDER, arrowcolor=FANUC_TEXT2)
        s.configure("TScrollbar", background="#e8e4e0", troughcolor=FANUC_BG,
                    arrowcolor=FANUC_TEXT2, relief="flat", width=10)
    ...
    def _create_menu(self):
        ...
        _m("View", [
            ...
            dict(label="Jog Mode: Cartesian", command=lambda: self._set_jog_mode(JOG_MODE_CARTESIAN)),
        ])
        _m("Help", [
    ...
    def _create_sidebar(self, parent) -> ctk.CTkFrame:
        sidebar = ctk.CTkFrame(parent, fg_color=_CARD_BG, width=64,
                               corner_radius=10, border_width=1, border_color=_BORDER)
        sidebar.pack_propagate(False)

        _BTN = dict(width=48, height=32, corner_radius=8,
        ...
        for txt, fg, hov, cmd, tip in [
            ...
            b = ctk.CTkButton(sidebar, text=txt, fg_color=fg, hover_color=hov,
                              text_color=FANUC_TEXT, command=cmd, **_BTN)
        ...
        estop = ctk.CTkButton(sidebar, text="STOP",
                              fg_color="#cc0000", hover_color="#ff0000",
                              text_color="white", command=self._emergency_stop, **_BTN)
        ...
        ctk.CTkLabel(sidebar, text="SPD",
                     font=ctk.CTkFont("Segoe UI", 7, "bold"),
                     text_color=FANUC_ORANGE).pack()

        self.speed_override_var = tk.IntVar(value=SPEED_OVERRIDE_DEFAULT)
        ctk.CTkSlider(
            sidebar,
            from_=SPEED_OVERRIDE_MIN, to=SPEED_OVERRIDE_MAX,
    ...
    def _add_tooltip(widget, text: str):
        ...
        def show(e):
            ...
            tk.Label(tip, text=text, bg="#ffffcc", fg=FANUC_TEXT,
    ...
    def _create_dashboard_tab(self):
        main = ctk.CTkScrollableFrame(self.main_tab, fg_color=FANUC_BG,
                                      corner_radius=0,
        ...
        ctk.CTkLabel(port_row, text="COM Port:",
                     font=ctk.CTkFont("Segoe UI", 11, "bold"),
                     text_color=_ACCENT).pack(side="left", padx=5)

        self.port_combo = ttk.Combobox(port_row, width=22, font=("Segoe UI", 11))
        self.port_combo.pack(side="left", padx=5)

        ctk.CTkButton(port_row, text="REFRESH",
                      fg_color=FANUC_GRAY, hover_color="#6a6560",
                      text_color=FANUC_TEXT, width=80, height=32, corner_radius=8,
                      font=ctk.CTkFont("Segoe UI", 9, "bold"),
                      command=self._refresh_ports).pack(side="left", padx=5)

        self.connect_btn = ctk.CTkButton(
            port_row, text="CONNECT",
            fg_color=FANUC_BLUE, hover_color="#6a94d8",
            text_color=FANUC_TEXT, width=110, height=34, corner_radius=8,
            font=ctk.CTkFont("Segoe UI", 11, "bold"),
        ...
        self.scan_btn = ctk.CTkButton(
            port_row, text="SCAN SERVOS",
            fg_color=FANUC_ORANGE, hover_color="#d4995a",
            text_color=FANUC_TEXT, width=120, height=34, corner_radius=8,
            font=ctk.CTkFont("Segoe UI", 11, "bold"),
            state="disabled",
        ...
        for txt, cmd, fg, hov in [
            ...
            ctk.CTkButton(qa_row, text=txt, fg_color=fg, hover_color=hov,
        ...
        for i in range(6):
            ctk.CTkLabel(jt_inner, text=f"J{i + 1}:",
                         font=ctk.CTkFont("Consolas", 11, "bold"),
                         text_color=_ACCENT).pack(side="left", padx=(12, 2))
            lbl = ctk.CTkLabel(jt_inner, text="0.0",
    ...
    def _create_block_tab(self):
        ...
        sb = ctk.CTkScrollbar(canvas_frame, command=self.program_canvas.yview,
        ...
        for txt, cmd, fg, hov in [
            ...
            ctk.CTkButton(btn_bar, text=txt, fg_color=fg, hover_color=hov,
                          text_color=FANUC_TEXT, width=72, height=32, corner_radius=8,
                          font=ctk.CTkFont("Segoe UI", 10, "bold"),
    ...
    def _create_coordinates_tab(self):
        ...
        ctk.CTkButton(btn_row, text="REFRESH",
                      fg_color=FANUC_BLUE, hover_color="#6a94d8",
                      text_color=FANUC_TEXT, width=90, height=34, corner_radius=8,
                      font=ctk.CTkFont("Segoe UI", 10, "bold"),
                      command=self._update_tool_coordinates).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="ZERO",
                      fg_color=FANUC_ORANGE, hover_color="#d4995a",
                      text_color=FANUC_TEXT, width=90, height=34, corner_radius=8,
                      font=ctk.CTkFont("Segoe UI", 10, "bold"),
                      command=self._reset_tool_coordinates).pack(side="left", padx=5)
    ...
    def _coord_row(parent, label: str, variable: tk.StringVar, color: str):
        ...
        ctk.CTkLabel(row, text=label, font=ctk.CTkFont("Consolas", 13, "bold"),
                     text_color=color, width=40).pack(side="left", padx=10)
        ctk.CTkLabel(row, textvariable=variable,
                     font=ctk.CTkFont("Consolas", 17, "bold"),
                     text_color=FANUC_TEXT, fg_color="#f0ece8",
                     corner_radius=6, width=160, height=34).pack(side="left", padx=10, fill="x", expand=True)
    ...
    def _create_log_panel(self, parent):
        ...
        ctk.CTkButton(btn_row, text="CLEAR LOG",
                      fg_color="#6a6560", hover_color="#4a4540",
                      text_color="white", width=80, height=24, corner_radius=6,
                      font=ctk.CTkFont("Segoe UI", 8, "bold"),
    ...
    def _draw_axis_indicator(self, canvas: tk.Canvas, axis: int, state: str):
        ...
        canvas.create_text(13, 13, text=f"J{axis + 1}", fill="white",
        ...
        for j in range(6):
            ...
            if mid in motor_data_dict:
                ...
                if d.position is not None:
                    state = "warning" if (d.is_overheating() or (d.load and d.load > 80)) else "active"
            if j in self.axis_indicators:
    ...
    def _update_tool_coordinates(self):
        ...
        vals = {
            "X":  f"{end_pos[0]:.2f}", "Y":  f"{end_pos[1]:.2f}", "Z":  f"{end_pos[2]:.2f}",
            "Rx": f"{angles[4]:.2f}", "Ry": f"{angles[3]:.2f}",  "Rz": f"{angles[0]:.2f}",
        }
        self.tool_x_var.set(vals["X"]);  self.tool_y_var.set(vals["Y"])
        self.tool_z_var.set(vals["Z"]);  self.tool_rx_var.set(vals["Rx"])
        self.tool_ry_var.set(vals["Ry"]); self.tool_rz_var.set(vals["Rz"])
    ...
    def _reset_tool_coordinates(self):
        for var in (self.tool_x_var, self.tool_y_var, self.tool_z_var,
                    self.tool_rx_var, self.tool_ry_var, self.tool_rz_var):
            var.set("0.00")
    ...
    def _adjust_speed(self, delta):
        new_val = max(SPEED_OVERRIDE_MIN, min(SPEED_OVERRIDE_MAX,
                      self.speed_override + delta))
